...
import { reportedSpeechTopics, reportedSpeechExercises } from '../data/reportedSpeech.js';
import { shuffle } from '../utils/helpers.js';
import { saveExerciseResult, saveSession } from '../utils/storage.js';
import ExerciseCard from './ExerciseCard.jsx';

export default function ReportedSpeechView() {
  const [expanded, setExpanded] = useState(null);
  const [quizState, setQuizState] = useState("setup");
  const [selTopics, setSelTopics] = useState(reportedSpeechTopics.map(t => t.id));
  ...
  const available = reportedSpeechExercises.filter(e => selTopics.includes(e.tenseId)).length;
  ...
  function handleNext() {
    if (qIdx + 1 >= questions.length) { setQuizState("done"); saveSession({ score, total: questions.length, mode: "reported-speech" }); }
    else setQIdx(i => i + 1);
  }
    ...
    <div>
      <div style={{ marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", gap: "0.6rem", alignItems: "center", marginBottom: "1rem", paddingBottom: "0.4rem", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <span style={{ fontSize: "0.62rem", letterSpacing: "0.25em", textTransform: "uppercase", color: "#9896A0" }}>Mowa Zależna — Teoria i przykłady</span>
        </div>
        <div style={S.tip}>
          <span style={S.tipLabel}>💡 Czym jest mowa zależna?</span>
          Przekazujemy to, co ktoś powiedział — bez cudzysłowu. Trzy kluczowe zmiany: <strong>(1) backshift</strong> — cofamy czas, <strong>(2) zaimki</strong> — dostosowujemy do perspektywy, <strong>(3) wyrażenia czasu i miejsca</strong> — now→then, here→there, today→that day. <strong>Po 'if' i 'where' NIE ma inwersji!</strong>
        </div>
        <div style={{ marginTop: "1rem" }}>
          {reportedSpeechTopics.map(t => {
                ...
                {open && (
                  <div style={S.cardBody} onClick={e => e.stopPropagation()}>
                    ...
                    <div style={{ marginBottom: "0.8rem" }}>
                      <div style={S.sLabel}>Bezpośrednia → Zależna</div>
                      {t.pairs.map((p, i) => (
                        <div key={i} style={{ display: "flex", gap: "0.5rem", alignItems: "center", flexWrap: "wrap", marginBottom: "0.4rem" }}>
                          <div style={{ ...S.exBox("question"), flex: 1, fontStyle: "italic" }}>💬 {p.direct}</div>
                          <span style={{ color: "#9896A0", flexShrink: 0 }}>→</span>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "1.5rem" }}>
        <div style={{ fontSize: "0.62rem", letterSpacing: "0.25em", textTransform: "uppercase", color: "#9896A0", marginBottom: "1rem" }}>🧠 Ćwiczenia — Mowa Zależna</div>
        {quizState === "setup" && (
          <div style={S.card}>
            <div style={{ fontSize: "0.9rem", fontWeight: 600, marginBottom: "1rem" }}>⚙️ Wybierz tematy do ćwiczenia</div>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end" }}>
              <button style={S.restartBtn} disabled={available === 0} onClick={() => startQuiz(shuffle(reportedSpeechExercises.filter(e => selTopics.includes(e.tenseId))).slice(0, Math.min(count, available)))}>Rozpocznij →</button>
            </div>
          </div>
        )}
        {quizState === "playing" && (
        ...
        {quizState === "done" && (
          <div style={S.scoreBox}>
            ...
            <div style={{ color: "#9896A0", fontSize: "0.85rem", marginTop: "0.3rem" }}>{score === questions.length ? "Perfekcyjnie! Mowa zależna opanowana." : score / questions.length >= 0.7 ? "Świetnie! Jeszcze trochę ćwiczeń." : "Wróć do teorii i spróbuj ponownie."}</div>
            {wrongCount > 0 && (
